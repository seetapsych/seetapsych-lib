...
def filename_of_url(url: str, response: HTTPResponse | None) -> str | None:
    ...
    raw_filename: str | tuple[Any, ...] | None = (
        None if response is None else response.headers.get_param("filename", header="Content-Disposition")
    )
    if raw_filename is not None:
        if isinstance(raw_filename, tuple):
            return str(raw_filename[-1])
        return str(raw_filename)

    # no filename found in headers, extract from url
    parsed_url = urlparse(url)
    extracted = os.path.basename(unquote(parsed_url.path))
    if not extracted or extracted == "." or extracted == "..":
        # try extract file=xxx or filename=xxx in url query
        extracted_final: str | None = None
        query = parse_qs(parsed_url.query)
        if query:
            for name in ["filename", "file"]:
                if name in query:
                    extracted_final = query[name][0]
                    break
...
def download_file(
    ...
    if scheme in {"http", "https"}:
        return download_http_file(**common_kwargs)
    elif scheme in {"ftp"}:
        return download_ftp_file(**common_kwargs)
    else:
    ...
    for attempt in range(max_retries):
        try:
            with urlopen(url, timeout=timeout_seconds) as resp:
                http_resp: HTTPResponse = resp  # type: ignore[assignment]
                current_output_path = extract_output_path(url, output, http_resp)
                ...
                if os.path.exists(current_output_path):
                    if overwrite:
                        ...
                        logger.warning(
                            f"File {os.path.basename(current_output_path)} already exists,"
            ...
            if attempt + 1 < max_retries:
                # retry in next time
                logger.error(
                    f"Tried [{attempt + 1}/{max_retries}] download {url} failed."
                ...
                raise RuntimeError(
                    f"Failed to download {current_output_name} from {url} after {max_retries} retries"
                ) from e
    ...
    *,
    ...
    with open(output_path, "wb") as f:
        with tqdm(
            ...
            ftp.retrbinary(f"RETR {remote_path}", callback, blocksize=buffer_size_bytes)
...
def _safe_zip_extract(zf: zipfile.ZipFile, output_dir: str):
    output_dir_abs = os.path.abspath(output_dir)
    safe_names: list[str] = []
    for member in zf.namelist():
        member_path = os.path.abspath(os.path.join(output_dir, member))
        if not _is_safe_path(output_dir_abs, member_path):
...
def _safe_tar_extract(tf: tarfile.TarFile, output_dir: str):
    output_dir_abs = os.path.abspath(output_dir)
...
def extract_file(file: str, output_dir: str | None = None):
    ...
    if basename.endswith(".zip"):
        with zipfile.ZipFile(file, "r") as f:
            _safe_zip_extract(f, output_dir)
    elif basename.endswith(".tar"):
        with tarfile.open(file, "r") as f:
            _safe_tar_extract(f, output_dir)
