...
class Entry(CustomBaseModel):
    package: str | None = Field(None, examples=["x.y.z"], description="Package from which the method is called")
    method: str = Field(
        examples=["a.b.c.func"],
...
class CloudModel(CustomBaseModel):
    ...
    model_id: str = Field(description="Model ID in the hosting repository")
    revision: str = Field("", description="Optional revision/tag/commit to download")
    repo_type: str = Field(
        "",
        description="Repository type for the cloud host (e.g., Hugging Face: model/dataset/space)",
    )
    allow_patterns: list[str] | None = Field(
        None, description="Optional allow patterns for file filtering (Hugging Face only)"
    )
    ignore_patterns: list[str] | None = Field(
        None, description="Optional ignore patterns for file filtering (Hugging Face only)"
    )
    index: str = Field(
        "",
...
class DownloadModel(CustomBaseModel):
    index: str = Field(description="Output filename used as the model cache index, for existence checks.")
    url: str = Field(description="Model download FTP or HTTP/S URL. HTTP/S is recommended")
    md5: str = Field("", description="MD5 checksum of the downloaded model file")
    sha256: str = Field("", description="SHA256 checksum of the downloaded model file")
    unpack: bool = Field(
        False,
        description=("Whether the downloaded file is compressed. If true, it will be decompressed automatically."),
    )
    contains: list[str] = Field([], description="Extra file/dir path for existence check. Only work with unpack = True")
...
class Model(Entity):
    ...
    Or, you can directly download it using the `download_url`
    and verify it using the `download_md5` in `download`.
    """

    usage: str = Field("", description="Specifies the model to use within a multi-model algorithm package")

    recommended: bool = Field(False, description="If true, this model is used by default")

    cloud: CloudModel | None = Field(None, description="")

    download: DownloadModel | None = Field(None, description="")

    entry: Entry | None = Field(
        None,
        description=("Python entry for loading this model. The entry should return @ref seetapsych_lib.api.Model"),
    )
...
class Package(Entity):
    ...
    priority: int = Field(
        0,
        description=(
            "Selection priority when multiple packages provide the same attribute. Higher value wins. Defaults to 0"
        ),
    )
    entry: Entry = Field(
    ...
    models: list[Model] = Field(description="Available models that can be switched as needed")
...
class ModuleSpec(Entity):
    requirements: list[str] = Field([], description="Python package requirements")

    refs: list[GitRef] = Field([], description="Python package to install from git")
...
class Module(CustomBaseModel):
    ...
    packages: list[Package] = Field([], description="Packages that can be loaded")
