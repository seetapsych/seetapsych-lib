...
def load_local_module(f: str | bytes | IO[str] | IO[bytes], extension: str | None = None) -> LoadedModule | None:
    ...
    try:
        ...
        if isinstance(f, bytes):
            f_display = f.decode(encoding="utf-8", errors="replace")
        else:
            f_display = f
        logger.warning(f"Can not parse module config: {f_display}.\n[Exception]: {e}")
        return None

    if isinstance(f, bytes):
        filename_src = f.decode(encoding="utf-8")
    elif isinstance(f, str):
        filename_src = f
    else:
        filename_src = "<anonymous>"

    return LoadedModule(source=filename_src, module=module)
