...
from seetapsych_lib.runtime.actions import call_entry, install_requirements
from seetapsych_lib.utils.defer import defer
from seetapsych_lib.utils.dirs import appdirs
from seetapsych_lib.utils.download_file import download_file, extract_file
...
def default_cache_dir() -> str:
    cache_dir = os.environ.get("SEETAPSYCH_CACHE_DIR", "") or appdirs.user_cache_dir
...
def entity_cache_dir(entity: schema.Entity, cache_dir: str | None = None) -> str:
...
class WrapperModel(api.UsageModel):
    def __init__(self, model: api.Model, *, usage: str = "", metadata: dict[str, Any] | None = None):
        self.__usage = usage
        self.__model = model
...
class LocalModel(api.UsageModel):
    def __init__(self, path: str, *, usage: str = "", metadata: dict[str, Any] | None = None):
        self.__usage = usage
        self.__path = path
...
class CloudModel(api.UsageModel):
    ...
    def __marker_matches(self, marker: dict[str, Any]) -> bool:
...
def _download_huggingface(cfg: schema.CloudModel, cache_dir: str) -> str:
    ...
    if repo_type:
        hf_kwargs["repo_type"] = repo_type
    if cfg.allow_patterns is not None:
        hf_kwargs["allow_patterns"] = cfg.allow_patterns
    if cfg.ignore_patterns is not None:
        hf_kwargs["ignore_patterns"] = cfg.ignore_patterns
    path = snapshot_download(**hf_kwargs)
    return str(path)
...
def _download_modelscope(cfg: schema.CloudModel, cache_dir: str) -> str:
    ...
    if find_spec("modelscope") is None:
...
def _download_aistudio(cfg: schema.CloudModel, cache_dir: str) -> str:
    ...
    if find_spec("aistudio_sdk") is None:
    ...
    revision = (cfg.revision or "").strip() or None
    aistudio_kwargs: dict[str, Any] = {
        "repo_id": cfg.model_id,
        "revision": revision or "master",
        "local_dir": cache_dir,
    }
    if cfg.allow_patterns is not None:
        aistudio_kwargs["allow_patterns"] = cfg.allow_patterns
    if cfg.ignore_patterns is not None:
        aistudio_kwargs["ignore_patterns"] = cfg.ignore_patterns
    path = snapshot_download(**aistudio_kwargs)
    return str(path)
...
def build_model_with_entry(
    entry: schema.Entry, *, usage: str = "", cache_dir: str | None = None, metadata: dict[str, Any] | None = None
) -> api.UsageModel:
    entry_kwargs: dict[str, Any] = {"cache_dir": cache_dir}
    return WrapperModel(call_entry(entry, entry_kwargs), usage=usage, metadata=metadata)


def build_model(model: schema.Model, *, cache_dir: str | None = None) -> api.UsageModel:
    model_dir = entity_cache_dir(model, cache_dir=cache_dir)

    bm_kwargs: dict[str, Any] = {"usage": model.usage, "cache_dir": model_dir, "metadata": model.metadata}

    if model.entry is not None:
        return build_model_with_entry(model.entry, **bm_kwargs)

    if model.download is not None:
        return build_model_with_download(model.download, **bm_kwargs)

    if model.cloud is not None:
        return build_model_with_cloud(model.cloud, **bm_kwargs)

    raise RuntimeError("The model configuration must at least provide cloud, download, or entry.")


def exists_model(model: schema.Model, *, cache_dir: str | None = None) -> bool:
    try:
