...
from fabopsy_lib.runtime.module import default_config_dir
from fabopsy_lib.utils.loader import load as load_config
from fabopsy_lib.utils.loader import map_parser as _loader_map_parser
from fabopsy_lib.runtime.actions import install_requirements, install_module_requirements
from fabopsy_lib.runtime.model import build_model, default_cache_dir
from fabopsy_lib.utils.download_file import download_file
...
def _ensure_fabopsy_configs() -> object:
    try:
        return importlib.import_module('fabopsy_configs')
    except ImportError:
        pass

    logger.info('fabopsy_configs not found, attempting to install...')
    try:
        install_requirements(['fabopsy-configs'])
    except Exception as e:
        logger.error(f'Failed to install fabopsy_configs: {e}')
        sys.exit(1)

    try:
        return importlib.import_module('fabopsy_configs')
    except ImportError as e:
        logger.error(f'Failed to import fabopsy_configs after install: {e}')
        sys.exit(1)
...
def download_configs(args: argparse.Namespace) -> None:
    ...
    configs_pkg = _ensure_fabopsy_configs()
    from fabopsy_configs import ConfigInfo
    configs_list: list[ConfigInfo] = getattr(configs_pkg, 'configs', [])

    if not configs_list:
        logger.warning('No configs found in fabopsy_configs.')
        return
    ...
    with tempfile.TemporaryDirectory(prefix='fabopsy_dl_') as tmpdir:
...
def main():
    parser = argparse.ArgumentParser(
        description="Fabopsy configuration manager",
    )
    ...
    download_parser = subparsers.add_parser(
        "download",
        help="Download and install all configs from fabopsy_configs",
    )
